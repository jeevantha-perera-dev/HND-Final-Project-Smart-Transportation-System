...
async function findTripForRoute(
  ...
  for (const code of routeLookupVariants(routeNumber)) {
    ...
    const any = await searchTrips({ routeCode: code, limit: 12, ...(dayWindow ?? {}) });
...
export async function enrichBusResultsWithTrips(
  ...
  for (const r of results) {
    ...
    try {
      const pick = await findTripForRoute(r.routeNumber, originLabel, destLabel, travelDateKey);
      if (pick) {
        const short = pick.shortRouteId?.trim();
        out.push({
          ...r,
          id: pick.id,
          tripId: pick.id,
